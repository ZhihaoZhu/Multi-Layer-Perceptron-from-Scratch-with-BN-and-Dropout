...
def forward_pass(X, W, b, layer, activation=Sigmoid()):
    pre_act = X @ W + b
    BN_output = BN_forward(pre_act,1,0,layer)
    post_act = activation.forward(BN_output)
...
def backwards(delta, W, b, layer, activation_deriv=sigmoid_deriv):
    ...
    dN, grad_gamma, grad_beta = BN_backward(delta_pre, cache, layer)
    grad_W = X.transpose() @ dN
    grad_b = np.sum(dN, axis=0, keepdims=True)
    grad_X = dN @ W.transpose()
...
for itr in range(max_iters):
    ...
    for xb, yb in batches:
        ...
        # backward
        delta1 = p - yb
        delta2, grad_W3, grad_b3, grad_gamma3, grad_beta3 = backwards(delta1, W3, b3, layer=3, activation_deriv=linear_deriv)
        delta3, grad_W2, grad_b2, grad_gamma2, grad_beta2 = backwards(delta2, W2, b2, layer=2, activation_deriv=sigmoid_deriv)
        _, grad_W1, grad_b1, grad_gamma1, grad_beta1 = backwards(delta3, W1, b1, layer=1, activation_deriv=sigmoid_deriv)
