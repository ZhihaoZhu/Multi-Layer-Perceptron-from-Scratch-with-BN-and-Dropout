...
class Tanh(Activation):
    ...
    def forward(self, x):
        sig = lambda p: (1-np.exp(-2*p)) / (1 + np.exp(-2*p))
        res = sig(x)
        return res
...
class SoftmaxCrossEntropy(Criterion):
    ...
    def forward(self, x, y):
        ...
        expo = lambda p : np.exp(p)
        result = expo(x)
        sum_nominator = np.sum(result)
        softmax = result/(sum_nominator + epsilon)

        log = lambda p : np.log(p)
        b = log(y)
        return -np.sum(softmax*b)
...
def random_normal_weight_init(d0, d1):
    W = np.random.uniform(-np.sqrt(6) / np.sqrt(d0 + d1),
                          np.sqrt(6) / np.sqrt(d0 + d1), d0 * d1).reshape((d0, d1))
    return W
